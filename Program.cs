    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                {
                    aptNoIndex = Array.FindIndex(words, t => t.Replace(",", "").Equals("No", StringComparison.InvariantCultureIgnoreCase));
                    if (aptNoIndex >= 0)
                    {
                        if (withComma)
                        {
                ...
                string others = Regex.Replace(System.Threading.Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(address.ToLower()), @"\s+", " ", RegexOptions.Multiline).TrimEnd();
